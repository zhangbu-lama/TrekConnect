...
const ManageTreks = () => {
  ...
  const { data: treks, isLoading, error, refetch } = useQuery({
    queryKey: ['treks'],
    queryFn: fetchTreks,
  });
  ...
  const addTrekMutation = useMutation({
    ...
    onError: (error) => {
      setToast({ show: true, message: `Failed to add trek: ${error.message}`, type: 'error' });
  ...
  const updateTrekMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['treks'] });
      setToast({ show: true, message: 'Trek updated successfully!', type: 'success' });
      setTimeout(() => setToast({ show: false, message: '', type: 'success' }), 3000);
    },
    onError: (error) => {
      setToast({ show: true, message: `Failed to update trek: ${error.message}`, type: 'error' });
      setTimeout(() => setToast({ show: false, message: '', type: 'error' }), 5000);
    },
  });
  ...
  const deleteTrekMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['treks'] });
      setToast({ show: true, message: 'Trek deleted successfully!', type: 'success' });
      setTimeout(() => setToast({ show: false, message: '', type: 'success' }), 3000);
    },
    onError: (error) => {
      setToast({ show: true, message: `Failed to delete trek: ${error.message}`, type: 'error' });
      setTimeout(() => setToast({ show: false, message: '', type: 'error' }), 5000);
    },
  });
  ...
  useEffect(() => {
    if (selectedTrek) {
      ...
      setImagePreviews(selectedTrek.images?.map(img => `${baseAPIurl}/Uploads/${img}`) || []);
      setIsModalOpen(true);
    } else {
      reset();
      setImagePreviews([]);
  ...
  const removeImage = (index) => {
    ...
    const newFiles = imagePreviews
      .filter((_, i) => i !== index)
      .map((_, i) => document.querySelector('input[type="file"]').files[i])
      .filter(file => file);
  ...
  const onSubmit = (data) => {
    ...
    if (selectedTrek) {
      updateTrekMutation.mutate({ _id: selectedTrek._id, data: formData });
    } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-gray-50 to-emerald-50 font-sans">
        ...
        >
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <h1 className="text-4xl font-bold text-gray-400">Trek Management</h1>
            <button
              onClick={() => {
                reset();
                setImagePreviews([]);
              ...
              <table className="w-full table-auto">
                <thead>
                  <tr className="bg-gradient-to-r from-teal-100 to-emerald-100 text-teal-800">
                    ...
                    <th className="py-4 px-6 text-left font-semibold text-sm uppercase tracking-wider">Images</th>
                  ...
                  {treks?.length > 0 ? (
                    treks.map((trek) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-gray-600">{trek.difficulty || 'N/A'}</td>
                        <td className="py-4 px-6 text-gray-600">{trek.place?.name || 'N/A'}</td>
                        <td className="py-4 px-6 text-gray-600">{trek.duration || 'N/A'}</td>
                        <td className="py-4 px-6 text-gray-600">{trek.rating || 'N/A'}</td>
                        <td className="py-4 px-6">
                          {trek.images?.length > 0 ? (
                            <div className="flex gap-2">
                              {trek.images.slice(0, 2).map((img, index) => (
                                <img
                                  key={index}
                              ...
                              {trek.images.length > 2 && (
                                <span className="text-gray-600">+{trek.images.length - 2}</span>
                              )}
                            </div>
                          ) : (
                    ...
                    <tr>
                      <td colSpan="7" className="py-12 text-center text-gray-500 text-lg">
                        No treks found. Add a new trek to get started!
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            >
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-3xl font-bold text-teal-800 tracking-tight">
                  {selectedTrek ? 'Edit Trek' : 'Add New Trek'}
                </h2>
                <button
              ...
              <form id="trek-form" onSubmit={handleSubmit(onSubmit)} className="space-y-12">
                {/* Trek Details */}
                <div className="space-y-6">
                  <h3 className="text-2xl font-semibold text-teal-800 border-b border-gray-200 pb-2">Trek Details</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Max Elevation</label>
                      <input
                ...
                {/* Contact Information */}
                <div className="space-y-6">
                  <h3 className="text-2xl font-semibold text-teal-800 border-b border-gray-200 pb-2">Contact Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                ...
                {/* Rating and Reviews */}
                <div className="space-y-6">
                  <h3 className="text-2xl font-semibold text-teal-800 border-b border-gray-200 pb-2">Rating & Reviews</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                ...
                {/* Images */}
                <div className="space-y-6">
                  <h3 className="text-2xl font-semibold text-teal-800 border-b border-gray-200 pb-2">Trek Images (Up to 4)</h3>
                      ...
                      <div className="relative flex items-center gap-3">
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <button
                            ...
                            disabled={addTrekMutation.isLoading || updateTrekMutation.isLoading}
                          >
                            {addTrekMutation.isLoading || updateTrekMutation.isLoading ? (
                ...
                <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
                  ...
                  <button
                    ...
                    disabled={addTrekMutation.isLoading || updateTrekMutation.isLoading}
                    aria-label={selectedTrek ? 'Update trek' : 'Add trek'}
                  >
                    {addTrekMutation.isLoading || updateTrekMutation.isLoading ? (
                    ...
                    {selectedTrek ? 'Update Trek' : 'Add Trek'}
                  </button>
                  <button
                    type="submit"
                    form="trek-form"
